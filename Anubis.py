...
#            ide.upload_file(self.portNo)
    ...
    def execute(self):
        instructionsToExecute = text.toPlainText()    
        #res = exec(instructions)
        text2.clear()
        try:
            if exec(instructionsToExecute)is None:
                text2.append("Done \n")
            instructionsToExecute  =instructionsToExecute.splitlines()
            for instruction in instructionsToExecute:
                if "def" in instruction :
                    leftParenthesesIndex = instruction.find("(") + 1
                    rightParenthesesIndex = instruction.find(")")
                    getParameters = instruction[leftParenthesesIndex:rightParenthesesIndex]
                    getParameters =getParameters.replace(" ", " ")
                    parametersList = getParameters.split(",")
                    functionNameBegin = instruction.find("def") + 4
                    functionNameEnd = instruction.find("(")
                    functionNameString = "result = " + instruction[functionNameBegin:functionNameEnd] + "("
                    if parametersList[0] != '':
                        inputParametersString, okClicked = QInputDialog.getText(self, "Parameters List", "Please enter function parametes seperated by 1 space")
                        if okClicked and inputParametersString != '':
                            inputParameters = inputParametersString.split(" ")
                        for param in inputParameters:
                            functionNameString = functionNameString + param + ","
                        functionNameString = functionNameString[:-1]
                    functionNameString = functionNameString + ")"
                    exec(functionNameString)
                    exec("text2.append('Output:  ' + str(result))")
        except:
            exec("text2.append('Something Went Wrong. Check your code')")
